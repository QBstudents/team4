...
##CREATING SITE SPECIFIC DATA FOR SITE BY SPECIES MATRIX
```{r}
...
plot.new()
site1 <- siteSpecies[1, ] #this uses Anna's site by species matrix
rac <- RAC(x = site1)
...
#conduct Chao1 analysis
S.chao1 <- function(x = ""){
  S.obs(x) + (sum(x == 1)^2)/(2 * sum(x == 2))
}

#attempting to conduct Chao2 analysis
...
##ANNA ATTEMPTS CONSTRAINED ORDINATION - STILL IN PROGRESS
```{r}
#attempting constrained ordination
kbs.dbrda <- dbrda(kbs.db ~ ., as.data.frame(sXspp.treat))
ordiplot(kbs.dbrda)
#calculate explained variation
dbrda.explainedvar1 <- round(kbs.dbrda$CCA$eig[1] /
                      sum(c(kbs.dbrda$CCA$eig, kbs.dbrda$CA$eig)), 3)*100
dbrda.explainedvar2 <- round(kbs.dbrda$CCA$eig[2] /
                      sum(c(kbs.dbrda$CCA$eig, kbs.dbrda$CA$eig)), 3)*100
#define plot parameters
par(mar = c(5,5,4,4) + 0.1)
#initiate plot
plot(scores(kbs.dbrda, display = "wa"), xlim = c(-1.3, 1.1),
     ylim = c(-1.1, 2.7), xlab = paste("dbRDA 1 (", dbrda.explainedvar1, "%)",
     sep = ""), ylab = paste("dbRDA 2 (", dbrda.explainedvar2, "%)", sep = ""),
...
points(scores(kbs.dbrda, display = "wa"), 
       pch = 19, cex = 3, bg = "gray", col = "gray")
text(scores(kbs.dbrda, display = "wa"),
     labels = row.names(scores(kbs.dbrda, display = "wa")))
#add treatment vectors
vectors <- scores(kbs.dbrda, display = "bp")
arrows(0,0, vectors[,1], vectors[,2],
       lwd = 2, lty = 1, length = 0.8, col = "red")
